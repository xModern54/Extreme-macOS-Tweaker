...
- Run `./Scripts/Build.sh` after every code change. It always creates an ARM64-only Release build in `.build/DerivedData`, builds both targets with Swift `-O` and C/ObjC `-O3`, embeds `RootTweakAction`, compiles `dqd` at `-O3`, adhoc-signs the app and helpers, and verifies ARM64 architecture. A successful build prints only `complete`; a failed build prints the complete Xcode log.
