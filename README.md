...
**Extreme macOS Tweaker** is a small native app for turning that extra layer off. You pick the features you don't use, review the plan, type your administrator password once, and apply. Fewer daemons, less memory, a snappier machine. The parts of macOS you actually live in stay put.
...
Walk through the system by category: Search & Intelligence, Siri, Messages, iCloud, Photos, Location, telemetry, and the rest. Keep only what you use.
...
Extreme macOS Tweaker remounts that volume, moves or deletes the apps you don't want, then blesses a new bootable snapshot. After a restart they are gone from the system you boot. Not hidden, not sitting in a trash that SIP will restore.
...
If you do not use them, they are just gigabytes on the disk. Extreme macOS Tweaker removes those unused system components so the space is yours again. macOS can always download a pack later if you change your mind.
...
Extreme macOS Tweaker is a power-user tool. It changes real system state.
...
- A snapshot change needs a restart. Extreme macOS Tweaker can reboot into a clean session so windows do not come back.
